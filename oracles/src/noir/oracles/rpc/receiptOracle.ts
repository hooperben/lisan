...
export const getReceiptOracle = async (
  ...
  const receipt = blockReceipts.find(
    (receipt) => receipt.transactionIndex === txId
  );
  if (!receipt) {
...
export function decodeGetReceiptArguments(args: NoirArguments): {
  ...
  console.log(args);
  assert(
    args.length === Enum.size(ARGS),
    `get_receipt requires ${Enum.size(ARGS)} arguments`
  );

  assert(args[ARGS.CHAIN_ID].length === 1, "chainId should be a single value");
  const chainId = Number(decodeField(args[ARGS.CHAIN_ID][0]));
  assert(
    args[ARGS.BLOCK_NUM].length === 1,
    "blockNumber should be a single value"
  );
